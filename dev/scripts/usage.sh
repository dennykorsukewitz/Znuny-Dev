...
show_usage() {
    ...
    print_command "$ZD_CMD cpanm <framework> [options] <Module::Name> ..." "Install CPAN modules in framework container (cpanm as root)"
...
show_usage_examples() {
    ...
    print_command "$ZD_CMD status --json" ""
    echo ""

    print_subheader "Dashboard:"
    echo ""
    print_command "$ZD_CMD dashboard start" ""
    print_command "$ZD_CMD dashboard stop" ""
    print_command "$ZD_CMD dashboard remove" ""
    ...
    print_command "$ZD_CMD delrebres dev" ""
    ...
    print_command "$ZD_CMD sql-schema dev" ""
    print_command "$ZD_CMD sql-initial-insert dev" ""
    print_command "$ZD_CMD cpanm dev CGI::Struct" ""
    print_command "$ZD_CMD cpanm dev --notest CGI::Struct" ""
...
show_usage_dev() {
    ...
    echo ""
    print_subheader "Experimental (local dashboard, Docker):"
    print_command "$ZD_CMD dashboard [start|stop|remove|build|restart|status]" "http://127.0.0.1:${DASHBOARD_PORT:-9999}/ — same data as status; refresh in browser only"
    echo ""
    ...
    print_command "$ZD_CMD dashboard start"          "Open http://127.0.0.1:${DASHBOARD_PORT:-9999}/ in browser"
    print_command "$ZD_CMD dashboard build"           "Rebuild image (Dockerfile); UI lives in dev/dashboard/public on the host — use restart after edits"
    print_command "$ZD_CMD dashboard restart"        "Restart dashboard container (same image)"
    echo ""
}
